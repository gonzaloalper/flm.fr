...
set.seed(1234567)
n <- 20
samp <- sample_flm_fr(n = n, argvalsX = seq(0, 1, l = 101), 
...
## Theoretical eigenfunctions of the covariance operator 
## (theoretical principal components)

set.seed(1234567)
large_samp <- sample_flm_fr(n = 1e4, argvalsX = seq(0, 1, l = 201), 
                            argvalsY = seq(0, 2, l = 301), beta = beta)
...
par(mfrow = c(1, 2))
plot(eigen_brownian(argvals = seq(0, 1, l = 201), j = 1:5))
plot(large_eigfun_X[1:5] * sign(large_eigfun_X$data[1:5, 2])) # Change signs
